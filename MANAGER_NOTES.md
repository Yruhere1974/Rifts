...
- Found while reviewing the tagged build, not yet fixed: `App.tsx` gives two sibling `<div>`s in `.pressure-section` the keys `instabilityPulse` and `roundPulse`, which are both 0 at deployment and collide whenever the two counters are equal. React warns, and because those keys exist precisely to force a remount so the flash and settle animations replay, sharing one is what stops that working. Also unfixed: two briefing labels in the north chamber (`brief-archive` at `-8,-12` and `brief-cache` at `-6,-13`) overlap permanently, and the master map's drawing is unreadable at phone width because it scales the whole undercroft to fit while the board crops to the action.
