...
| Glitter Boy     | Yes. Three routings from five dice into six systems, each firing with everything in it. Routing browns out every loose die showing lower, so the order decides what survives. Calibration doubles a matched system and adds a run's length. Held dice keep their faces into the next round. | Never played by a person. Defence exists only as Shield, and patrols have no facing to brace against. Per-system scoring appetites are the next Clever ingredient, deliberately not built yet. |
| Ley Line Walker | Yes. Chains alternate Channel and Resonance for as long as the hand sustains them, Exploit Opening is a wildcard, and length pays 1, 3, 6, 10. The hand carries itself but re-forms two fewer cards than it holds, so a long chain is bought with the round after it.                       | Never played by a person. A weave of five pays 15 against mission constants tuned when the ceiling was 3.                                                                                      |
| Techno-Wizard   | Yes. Four markers into a frame of seven sockets, each placement choosing its action and its position. +1 for each built neighbour, priming, and bolted sockets that stand into the next round so a machine accumulates.                                                                     | Never played by a person. Ports as a concept distinct from frame adjacency are not built, and may not be needed.                                                                               |
